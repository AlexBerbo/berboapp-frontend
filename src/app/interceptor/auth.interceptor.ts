    ...
    return next.handle(this.addAuthorizationTokenHeader(request, localStorage.getItem(Key.TOKEN)))
      .pipe(
        catchError((error: HttpErrorResponse) => {
          if (error instanceof HttpErrorResponse && error.status === 401 && error.error.reason.includes('You need to login again!')) {
    ...
    if (this.isTokenRefreshing === false) {
      ...
      return this.userService.refreshToken$().pipe(
        switchMap((response) => {
          ...
          console.log('New Token: ', response.data.jwt_token);
